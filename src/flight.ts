...
//
// A hit test claiming presses on butterflies. CLAUDE.md requires that dragging
// never swallow a touch, and input.ts has the hook ready — but there is no touch
// yet (step 9), so registering it now would only turn a third of the window into
// dead space where nothing happens and the widget cannot be moved. It belongs
// with the gesture it protects.
//
...
export const FLIGHT = {
  ...
  // reason for the treatment below is that the mixed cases have to stay
 ...
 */
export function poseTable(): { rest: WingPose; glide: WingPose; beat: WingPose[] } {
  const B = FLIGHT.beat;
...
export function rebuildFlightPoses(): void {
  ...
  setWingPoses(table.rest, table.glide, table.beat, FLIGHT.beat.camera);
  rebuildPlanes();
}

// --- the planes ------------------------------------------------------------
//
// Derived once and held, rather than recomputed per butterfly per frame. That
// is not only for speed: a plane's `look.key` is the tile cache key, and it says
// only which plane it is. Every constant that changes what a plane *looks* like
// is therefore a `rebuild` knob — dragging one throws the whole tile cache away
// through setWingPoses and this table is rebuilt with it. A depth constant that
// was not a rebuild knob would leave stale art behind under a live key, which is
// the one way this cache can lie.
...
export function planeCount(): number {
  return FLIGHT.depth.edges.length + 1;
}

/** How far back a plane sits, as 0 at the glass and 1 at the back wall. */
function depthU(depth: number): number {
  const n = planeCount();
...
export function planeOf(bucketsAgo: number): number {
  const edges = FLIGHT.depth.edges;
...
export function planePhases(plane: number): number {
  const full = Math.max(2, Math.round(FLIGHT.beat.phases));
  const far = Math.max(2, Math.min(full, Math.round(FLIGHT.depth.farPhases)));
  const n = Math.round(full * Math.pow(far / full, depthU(plane)));
  return Math.min(full, Math.max(2, n));
}

function rebuildPlanes(): void {
  const D = FLIGHT.depth;
  const n = planeCount();
  planes = [];
  for (let p = 0; p < n; p++) {
    const u = depthU(p);
    planes.push({
      ...
      scale: Math.round(FLIGHT.wingspan * Math.pow(D.farScale, u) * 100) / 100,
      phases: planePhases(p),
      rest: clamp(lerp(FLIGHT.rest.fraction, D.farRest, u), 0, 1),
      look:
        p === 0
          ? NEAR
          : {
...
function planeAt(index: number): Plane {
 ...
 */
function boundsAt(out: Rect, base: Rect, depth: number): Rect {
  const k = 1 - FLIGHT.depth.farShrink * depthU(depth);
  out.w = base.w * k;
  out.h = base.h * k;
...
export function stepFlight(dt: number, t: number, bounds: Rect): void {
  reconcile(bounds);
...
function settleDepth(dt: number, bounds: Rect): void {
  const ease = dt > 0 ? 1 - Math.exp(-dt / Math.max(0.01, FLIGHT.depth.easeSec)) : 0;
  const last = planeCount() - 1;
  for (const f of flyers) {
    const target = clamp(planeOf(f.bucketsAgo), 0, last);
    if (ease > 0) {
      f.depth += (target - f.depth) * ease;
      if (Math.abs(target - f.depth) < 0.002) f.depth = target;
    }
...
function stepFlyer(f: Flyer, dt: number, t: number, bounds: Rect): void {
  ...
  const size = Math.pow(FLIGHT.depth.farScale, depthU(f.depth));
  ...
  for (const g of flyers) {
    if (g === f || g.lookPlane !== f.lookPlane) continue;
    const dx = f.x - g.x;
    const dy = f.y - g.y;
...
export function drawFlight(ctx: CanvasRenderingContext2D, dpr: number): void {
  ...
  order.sort((a, b) => (a.lookPlane !== b.lookPlane ? b.lookPlane - a.lookPlane : a.y - b.y));
...
function edgeKnob(i: number, min: number, max: number): Knob {
  return {
    ...
    step: 1,
    rebuild: true,
    get: () => FLIGHT.depth.edges[i],
    set: (v) => {
      FLIGHT.depth.edges[i] = Math.round(v);
    },
  };
}
...
export function flightKnobs(): Knob[] {
  ...
  return [
    ...
    // Depth. Every one of these rebuilds, including the ones that only look
    // like simulation: a plane's tile cache key says nothing but which plane it
    // is, so the art behind that key has to be thrown away whenever any of them
    // moves. `easeSec` is the exception and the only one here that is purely
    // motion.
    //
    // The edges are ages in buckets and must stay in order — an edge dragged
    // past its neighbour empties the plane between them, which the F9 overlay's
    // `depth:` line shows as a zero. Setting them to 1, 2, 3, 4 gives
    // CLAUDE.md's one-bucket-per-plane model exactly.
    ...FLIGHT.depth.edges.map((_, i) =>
      edgeKnob(i, i === 0 ? 1 : 2, i === FLIGHT.depth.edges.length - 1 ? 60 : 40),
    ),
    knob("depth", DEEP, "farScale", 0.2, 1, 0.01, true),
    knob("depth", DEEP, "farShrink", 0, 0.7, 0.01, true),
    knob("depth", DEEP, "farRest", 0, 1, 0.01, true),
    knob("depth", DEEP, "farPhases", 2, 24, 1, true),
    knob("depth", DEEP, "farHaze", 0, 0.8, 0.01, true),
    knob("depth", DEEP, "farShadowScale", 0.5, 4, 0.05, true),
    knob("depth", DEEP, "farShadowAlpha", 0, 2, 0.01, true),
    knob("depth", DEEP, "easeSec", 0.05, 4, 0.05),
...
// The depth constants do live in FLIGHT, so this one is only a cast.
const DEEP = FLIGHT.depth as unknown as NumberBag;
 ...
 * came from. Two blocks rather than one because the shading constants are not
 * motion and do not live in FLIGHT — pretending otherwise would produce a dump
 * that looks pasteable and isn't.
 */
